function loadcomplete(){
    ...
    var promise4 =  d3.csv("../static/data/wsb_lmkt_x.csv")
    var promise5 =  d3.csv("../static/data/wsb_mmkt_x.csv")
    var promise6 =  d3.csv("../static/data/wsb_smkt_x.csv")
    ...
    Promise.all(promises).then(data => {
        ...
        var trace8 = {
            ...
            x: filpowsmall.map(dataday => dataday.date),
            y: filpowsmall.map(dataday => dataday.Sent_Count),
            yaxis:'y2',
            line: {color: "#212121"}
        }
...
function updatechartselect(myseldate) {
    ...
    d3.csv("../static/data/wsb_lmkt_x.csv").then((data) => {
    ...
    d3.json(`/ml/wsb/${myseldate}`).then(data =>{   
...
function updatechartselect_mid(myseldate) {
    ...
    mydate = new Date(mydate.setDate(mydate.getDate() - 7))
    mydate = mydate.toISOString().split('T')[0]
    d3.csv("../static/data/wsb_mmkt.csv").then((data) => {
    ...
    //   }
    // })
    d3.json(`/ml/wsb_mid/${myseldate}`).then(data =>{   
...
function updatechartselect_small(myseldate) {
    ...
    mydate = new Date(mydate.setDate(mydate.getDate() - 7))
    mydate = mydate.toISOString().split('T')[0]
    d3.csv("../static/data/wsb_smkt.csv").then((data) => {
    ...
    //   }
    // })
    d3.json(`/ml/wsb_small/${myseldate}`).then(data =>{   
